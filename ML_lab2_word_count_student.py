...
def numericSum(x, y):
    return x + y
wordCounts = wordPairs.reduceByKey(numericSum)
...
# TODO: Replace <FILL IN> with appropriate code
wordCountsCollected = (wordsRDD.map(lambda x : (x, 1)).reduceByKey(numericSum)
                       .collect())
...
def wordCount(wordListRDD):
    ...
    """
    return wordListRDD.map(lambda x: (x, 1)).reduceByKey(numericSum) 
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = shakeWordsRDD.map(lambda x: (x, 1)).reduceByKey(numericSum).takeOrdered(15, key=lambda (x, y): -y)
