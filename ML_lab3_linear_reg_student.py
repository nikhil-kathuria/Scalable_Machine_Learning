...
def parsePoint(line):
    ...
    return LabeledPoint(float(splitlist[0]), splitlist[- len(splitlist) + 1:])
...
def calcRMSE(labelsAndPreds):
    ...
    """
    sqrlist = labelsAndPreds.map(lambda (x, y) : squaredError(x, y))
    return pow((sqrlist.fold(0, add)) / sqrlist.count(), .5)
...
# TODO: Replace <FILL IN> with appropriate code
def averageYear(lpObject):
    """Calculates the average year value of input RDD.
    Args:
        LabelPoint Object
    Returns:
        float: The average year value.
    """
    return (lpObject
            .map(lambda x : x.label)
            .fold(0, add) / lpObject.count())

# Get average and use average as predicted to formulate RDD(label, predic) and finally calculate RMSE
...
def gradientSummand(weights, lp):
    ...
    """
    mul = weights.dot(lp.features) - lp.label
    return mul * lp.features
...
def linregGradientDescent(trainData, numIters):
    ...
    for i in range(numIters):
        ...
        gradlist = trainData.map(lambda x: gradientSummand(w, x))
        gradient = gradlist.reduce(add)
...
firstModel = LinearRegressionWithSGD.train(parsedTrainData, 
                                      iterations=numIters, step=alpha,
...
def twoWayInteractions(lp):
    ...
    for item1 in farray:
        for item2 in farray:
            interactions.append(float(item1) * float(item2))
    return LabeledPoint(lp.label, np.hstack((farray, interactions)))
